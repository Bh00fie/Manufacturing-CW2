...
########## Create two conveyors ##########
fourthConvey = QLabsConveyorStraight(qlabs)
num = fourthConvey.spawn_id_degrees(actorNumber = 70,
                                    location = [-0.12, positionSecond, 0.3],
                                    rotation = [0, 0, 0],
                                    scale = [1,1,1],
                                    configuration = 2)
fourthConvey.set_speed(0.1)
...
sixthConvey = QLabsConveyorStraight(qlabs)
num = sixthConvey.spawn_id_degrees(actorNumber = 73,
                                    location = [0.85, positionSecond, 0.3],
                                    rotation = [0, 0, 0],
                                    scale = [1,1,1],
                                    configuration = 2)
sixthConvey.set_speed(0.1)
...
### SUPPORT FOR SECOND CONVEYER
# Create a simple supports for the conveyors
fourthStand = QLabsBasicShape(qlabs)
fourthStand.spawn_id_and_parent_with_relative_transform(actorNumber = 798,
...
### ALUMINIUM
beamSensorSpawn2 = QLabsGenericSensor(qlabs)
...
beamSensorDrop2 = QLabsGenericSensor(qlabs)
...
def moveConveyors2(speed, stopped, notMovingTime):
    ...
    fourthConvey.set_speed(speed)       ## fouth conveyor
...
def recordResults(data, greenObjects):
    ...
    pd.DataFrame.to_csv(data, ("C:\\Users\\" + os.getlogin() + "\\Documents\\Quanser\\examples\\VirtualFactory\\Results_" + scriptStartTime + ".csv"), index=False)
    return data
...
def findObj():
    ...
    while True:
        ...
        # ALUMINIUM 
        _, hitSpawn2, _,_,_,_ = beamSensorSpawn2.test_beam_hit_widget()
        _, hitDrop2, _,_,_,_ = beamSensorDrop2.test_beam_hit_widget()
        ...
        if qarm2_State < 6:
            if elapsed_time(startTimeQarm2) > 1.5:
                ...
                if qarm2_State == 4:
                    stopped2, notMovingTime2 = moveConveyors2(0.1, stopped, notMovingTime)
        ...
        if emptyingBlue == 1:
            emptied = emptyBins(coverObject=BlueCover,startTime=startTimeBlue,timeToMove=1,timeOpened = 13)
            if emptied:
                emptyingBlue = 0
                blueObjects = 0
